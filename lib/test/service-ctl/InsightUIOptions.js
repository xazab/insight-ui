...
const NodeJsServiceOptions = require('@xazab/dp-services-ctl/lib/services/node/NodeJsServiceOptions');
...
class InsightUIOptions extends NodeJsServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    ...
    const defaultContainerOptions = {
      ...
      cmd: [
        '/insight/bin/xazabcore-node',
        'start',
      ],
      volumes: [...InsightUIOptions.defaultCustomOptions.container.volumes,
        `${configFile}:/insight/xazabcore-node.json`,
      ],
    };
  ...
  getConfigPath() {
    ...
    return path.join(tmpPath, 'bitcore-node-xazab.json');
  }
